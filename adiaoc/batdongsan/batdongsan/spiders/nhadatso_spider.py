...
class NhaDatSoSpider(RedisSpider):
    ...
    def start_requests(self):
        urls = [
            'https://nhadatso.com/nha-dat-ban/',
            'https://nhadatso.com/nha-dat-cho-thue/',
        ]
        for url in urls:
    ...
    def parse_list_and_paging(self, response):
        # Paging
        for line in response.css('#container > div.row > div.col-xs-12.col-sm-12.col-md-7.col-lg-7 > div.user-page.row.bg-w > div.bds_list > div'):
            item_url = line.css('.info-title a::attr(href)').extract_first()
            yield scrapy.Request(response.urljoin(item_url), callback=self.parse)

        # Paging
        for next_page in response.css('#page-regular > ul > li'):
    ...
    def parse(self, response):
        ...
        title = response.css('#container > div.row > div.col-xs-12.col-sm-12.col-md-7.col-lg-7 > div > div > div > h1::text').extract_first()
        description = response.css('#container > div.row > div.col-xs-12.col-sm-12.col-md-7.col-lg-7 > div:nth-child(1) > div > div > div.property-content.clearfix > div.property-extrainfo').extract_first()

        area_text = response.css('#container > div.row > div.col-xs-12.col-sm-12.col-md-7.col-lg-7 > div:nth-child(1) > div > div > div:nth-child(6) > span:nth-child(1)::text').extract_first()
        area = self.parse_area(area_text)

        price_text = response.css('div.price > span::text').extract()
        if price_text:
            price_text.append(response.css('div.price > span > b::text').extract_first())
        
        price_text = ' '.join(price_text) if price_text != None else ''
        ...
        contact_name = response.css('#container > div.row > div.col-xs-12.col-sm-12.col-md-7.col-lg-7 > div:nth-child(1) > div > div > div.box_lienlac.clearfix > div:nth-child(2) > a > h3::text').extract_first()
        ...
        post_type_text = response.css('#container > div.row > div.col-xs-12.col-sm-12.col-md-7.col-lg-7 > div:nth-child(1) > div > div > div.wrap.clearfix > div.title > h2 > a::text').extract_first()
